...
import { updateMealAction, type UpdateMealInput } from "./actions";
...
export function EditMealForm({ meal }: EditMealFormProps) {
  ...
  const [loggedAt, setLoggedAt] = useState(toDateString(meal.loggedAt));
  const [foodItems, setFoodItems] = useState<FoodItemDraft[]>(
    meal.foodItems.map((fi) => ({
      ...
      unit: fi.unit,
      calories: fi.calories != null ? String(fi.calories) : "",
      protein: fi.protein != null ? String(fi.protein) : "",
      carbs: fi.carbs != null ? String(fi.carbs) : "",
      fat: fi.fat != null ? String(fi.fat) : "",
    }))
  );
  ...
  function addFoodItem() {
    setFoodItems((prev) => [
      ...prev,
      { id: null, name: "", quantity: "", unit: "g", calories: "", protein: "", carbs: "", fat: "" },
    ]);
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const data: UpdateMealInput = {
      mealId: meal.id,
      name: mealName,
      loggedAt: loggedAt + "T12:00:00",
      deletedFoodItemIds: deletedIds,
    ...
    startTransition(async () => {
      const result = await updateMealAction(data);
      if (result.ok) {
